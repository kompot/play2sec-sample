...
class CommonsMailerPlugin(app: play.api.Application) extends MailerPlugin {
  ...
  private lazy val mailerInstance: MailerAPI = if (mock) {
    ...
    val smtpHost = app.configuration.getString("smtp.host").getOrElse(
      throw new RuntimeException("smtp.host needs to be set in " +
          "application.conf in order to use this plugin (or set smtp.mock to true)"))
  ...
  override lazy val enabled =
    !app.configuration.getString("apachecommonsmailerplugin").exists(_ == "disabled")

  override def onStart = mailerInstance
